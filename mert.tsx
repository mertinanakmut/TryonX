...
const App: React.FC = () => {
  ...
  const handleManualUpload = async (base64Image: string) => {
    ...
    if (!profileError) {
      const post: Partial<RunwayPost> = { userId: state.currentUser.id, userName: state.currentUser.name, userAvatar: state.currentUser.avatar_url, resultImage: base64Image, category: 'LIFESTYLE', vibe: 'AUTHENTIC', is_manual: true, trend_score: 0 };
  ...
  const handleTryOnExecute = async () => {
    ...
    try {
      ...
      if (state.currentUser) {
        ...
        const post: Partial<RunwayPost> = { userId: state.currentUser.id, userName: state.currentUser.name, userAvatar: state.currentUser.avatar_url, resultImage: resultUrl, garmentImage: state.garmentImage, category: state.category.toUpperCase(), vibe: advice.vibe.toUpperCase(), is_manual: false, trend_score: 50 };
        await supabase.from('runway_posts').insert([post]);
      }
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-cyan-500/30">
      ...
      <main>
        ...
        {state.view === 'home' && (
          <div className="flex flex-col items-center justify-center min-h-[80vh] text-center p-6 animate-in fade-in duration-1000 relative">
            <div className="flex items-center gap-4 group">
               <h1 className="text-8xl sm:text-9xl font-black mb-4 tracking-tighter">Tryon</h1>
               <NeuralLogo className="w-24 h-24 sm:w-32 sm:h-32 mb-4" />
            </div>
            <p className="text-gray-500 uppercase tracking-[0.4em] mb-12">Nöral Sentez & Stil Ağı</p>
            <button onClick={() => updateState({ view: 'studio' })} className="px-16 py-6 tryonx-gradient rounded-full font-black uppercase tracking-widest hover:scale-105 transition shadow-2xl relative z-10 group overflow-hidden">
